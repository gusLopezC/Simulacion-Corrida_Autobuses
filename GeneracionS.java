...
public class GeneracionS {
	...
	public void corrida(String nombreArchivo, int dia, int tretardo, int asientos) {
		...
		try {
			...
			for (int z = 0; z <= dia - 1; z++) {// dia
				...
				for (int i = 0; i < hora.length; i++) {
					...
					int Tdinero = (AsiCom * 48) + (AsiMed * 24) + (AsiIna * 12);
					System.out.println(String.format("%.2f", hora[i]) + "\t" + String.format("%.2f", horaReal)
							+ " \t     San Juan R      " + asientos + "\t " + AsientosTAle + "\t  " + AsiCom + "\t    "
		...
		for (int i = 0; i <= dia - 1; i++) {
			...
			wr.write("Dia" + condia+"\n");
			condia++;
			System.out.println("\tSan Juan R \t   " + arrayBolC[i] + "      " + arrayBolM[i] + "      " + arrayBolI[i]);
			wr.write("\tSan Juan R \t   " + arrayBolC[i] + "      " + arrayBolM[i] + "      " + arrayBolI[i]+"\n");
		}
		condia = 1;
		...
		for (int i = 0; i <= dia - 1; i++) {
			...
			System.out.println("\tSan Juan R       $" + arrayDinC[i] + "   $" + arrayDinM[i] + "   \t$" + arrayDinI[i]);
			wr.write("\tSan Juan R       $" + arrayDinC[i] + "   $" + arrayDinM[i] + "   \t$" + arrayDinI[i]+"\n");
		}
		condia = 1;
		...
		for (int i = 0; i <= dia - 1; i++) {
			System.out.println("Dia" + (condia) + "\tSan Juan R " + "	  \t" + arrayPro[i]+ "%");
			wr.write("Dia" + (condia) + "\tSan Juan R " + "	  \t" + arrayPro[i]+ "%\n");
			condia++;
